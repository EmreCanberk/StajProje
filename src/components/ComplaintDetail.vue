    ...
    <div v-else-if="complaint" class="detail-card">
      ...
      <div class="detail-section">
        ...
        <div class="message-content-wrapper">
          <p class="complaint-message" :class="{ expanded: isMessageExpanded }">{{ complaint.message }}</p>
          <button v-if="shouldShowReadMore" @click="toggleMessageExpansion" class="read-more-button">
            {{ isMessageExpanded ? 'Daha az göster' : 'Daha fazla oku' }}
          </button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    shouldShowReadMore() {
      return this.complaint && this.complaint.message && this.complaint.message.length > 200;
    }
  },
  async created() {
